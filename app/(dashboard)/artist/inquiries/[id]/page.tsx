...
import type { CustomOrderInquiry, InquiryMessage } from '@/types/database.types'
...
export default async function ArtistInquiryDetailPage({
  ...
  const { data: inquiry } = await supabase
    ...
    .select('*, customer:profiles(full_name), product:products(title)')
    .eq('id', params.id)
    .eq('artist_id', userId)
  ...
  const customerName = inquiry.customer?.full_name ?? 'this customer'
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              From {customerName} · Received {formatDate(inquiry.created_at)}
            </p>
          </div>
